...
def purchase_stock(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Create NewStockModel instance
            new_stock = NewStockModel(sku=sku, supplier=supplier, quantity=quantity, price=price, total_price=total_price)
            new_stock.save()

            # Update inventory quantity
            # inventory = get_object_or_404(InventoryModel, product_name=sku)
