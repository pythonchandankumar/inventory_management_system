    ...
    if request.method == 'POST':
        ...
        if customer_form.is_valid() and formset.is_valid():
            ...
            for form in formset:
                ...
                not_available = False
                inventory = get_object_or_404(InventoryModel, product_name=product)
                if quantity <= inventory.available_quantity:
                    cust_model = CustomerModel(customer=customer, contact=contact)
                    cust_model.save()
                    
                    model = SalesModel(customer=cust_model, product=product, quantity=quantity, price=price, total_price=total_price)
                    model.save()
